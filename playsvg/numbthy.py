...
def isprimeE(n,b):
	"""isprimeF(n) - Test whether n is prime or an Euler pseudoprime to base b."""
...
def factorPR(n):
	"""factorPR(n) - Find a factor of n using the Pollard Rho method.
	Note: This method will occasionally fail, for instance n=4."""
	for slow in [2,3,4,6]:
		...
		while i<numsteps:
			...
			fast = (fast*fast + 1) % n
			fast = (fast*fast + 1) % n
			if (gcd(fast-slow,n) != 1):
				return gcd(fast-slow,n)
	return 1
