...
for i in range(gridSize-1):
    for j in range(gridSize-2):
        ...
        faceGroup.append(buildPath( PathData().makeHull(boxPoints),{'style':'stroke:black;stroke-width:1;fill:'+checkerColors[(i+j)%2]} ))
j = gridSize-2
for i in range(gridSize-1):
...
##        lineTo(perspectiveGrid[i][j+1]).lineTo(perspectiveGrid[i+1][j+1]).lineTo(perspectiveGrid[i+1][j]).closePath()
        ...
        faceGroup.append(buildPath( PathData().makeHull(triPoints),{'style':'stroke:black;stroke-width:1;fill:'+checkerColors[(i+j)%2]} ))
