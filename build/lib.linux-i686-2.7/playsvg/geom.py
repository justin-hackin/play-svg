...
class Point:
    def __init__(self, xval=0, yval=0, fromVector=None):
        if (fromVector != None):  #for use with geosolver methods which use vectors 
            if (len(fromVector) == 2): 
                self.x = fromVector[0]
                self.y = fromVector[1]
            else:
    ...
    def strOnPlane(self): #used to output 3D points for OpenSCAD
        return "[" + aNum(self.x)+","+aNum(self.y) + ",0]"
    def strForPoly(self): #used to output 2D points for OpenSCAD
        return "[" + aNum(self.x)+","+aNum(self.y) + "]"
    ...
    def getVector(self):  #for use with geosolver methods which use vectors 
